...
- Residual OpenAPM **§10 security-depth** gaps (host-class credential scoping / AuthResolver; OpenAPM tar.gz-only container stays soft while zip + size/entry caps ship) — marketplace floor exists and does not by itself satisfy deferred `sc-*` reqs
