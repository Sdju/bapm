...
- Runtime host — opt-in пакет (пример: **Cursor** / `@b-apm/integration-cursor`); для известных hosts object-map `targets:` не обязателен (canonical fallback). Map — override / custom ([hosts](/guide/supported-hosts)).
