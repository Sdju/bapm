...
Также намеренно (не баги): ∩-pick vs APM first-wins, dual-read branding, OpenAPM-strict YAML anchors, bapm-only source discriminator `local` (default `.agents/local` / custom path + gitignore ensure; portable OpenAPM form remains `path:`), и **bapm-only personal overlay** `bapm.local.yml` (allowlisted settings merge; не часть OpenAPM wire и не `local:` source).
