...
- `bapm.yml` с `target: cursor` (или уже существующий манифест без dual-conflict).
...
- `Target detection is missing or ambiguous` / нет `.cursor/` → передайте `--target cursor`.
