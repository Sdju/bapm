/**
 * Acceptance helpers for manifest-target-integration-map.
...
export const configManifestGuidePath = join(
  repoRoot,
  "apps/docs/guide/config-manifest.md",
);
...
export function parseManifestDocument(input: unknown): {
  ...
  return (
    pickExport(
      ["parseManifestDocument", "parseManifest"],
      "Manifest parse",
    ) as (input: unknown) => { document: BapmManifest; warnings?: unknown[] }
  )(input);
}
...
/** Public helper retaining host→package map for future wiring (design §5). */
export function getDeclaredTargetIntegrationMap(): (
  manifest: BapmManifest,
) => Readonly<Record<string, string>> | undefined {
  return pickExport(
    ["declaredTargetIntegrationMap", "targetIntegrationMapOf"],
    "target integration map helper",
  ) as (manifest: BapmManifest) => Readonly<Record<string, string>> | undefined;
}

export function createTempProject(prefix = "bapm-tgt-map-"): TempProject {
