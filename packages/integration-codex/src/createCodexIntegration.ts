...
export function createCodexIntegration(options?: {
  ...
  return {
    ...
    async compile(primitives, context): Promise<CompileReport> {
      return compileCodexAgentsMd(primitivesList(primitives), context);
    },
...
function compileCodexAgentsMd(
  ...
  if (!rel || rel === ".." || rel.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`)) {
    throw new Error("Codex compile output must be a cwd-relative file path");
  }
  if (basename(outputPath) !== "AGENTS.md") {
    throw new Error("Codex compile output basename must be AGENTS.md");
  }

  const content = renderCodexAgentsMd(primitives);
...
function renderCodexAgentsMd(primitives: AttributedPrimitive[]): string {
  // Include instructions (compile-only path). Shared AGENTS.md with Cursor: last writer wins.
...
function materializeCodexAgent(args: {
  ...
  const destFile = join(cwd, ".codex", "agents", `${name}.toml`);
