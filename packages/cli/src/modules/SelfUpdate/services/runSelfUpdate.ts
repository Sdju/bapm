...
Upgrade path:
  npm i -g @b-apm/cli@<version>
  (or: npm update -g @b-apm/cli)
...
export async function runSelfUpdateCli(
  ...
  try {
    ...
    if (process.env.BAPM_SELF_UPDATE_DISABLE === "1") {
      console.error(
        `${deps.name}: self-update upgrade path disabled (BAPM_SELF_UPDATE_DISABLE=1). Use: npm i -g @b-apm/cli@latest`,
      );
    ...
    console.log(`Upgraded via npm i -g @b-apm/cli@${target}`);
    return { ok: true, exitCode: 0 };
  } catch (error) {
    const message =
...
function defaultNpmUpgrade(_pkg: string, version: string): Promise<void> {
  return new Promise((resolvePromise, reject) => {
    const child = spawn("npm", ["i", "-g", `@b-apm/cli@${version}`], {
      stdio: "inherit",
    });
