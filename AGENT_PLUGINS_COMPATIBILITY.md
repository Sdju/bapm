...
| `skills` | supported | Skills from conventional immediate skills/<name>/SKILL.md when plugin.json omits skills, or from exclusive plugin.json skills declaration (omit vs [] vs list; fail-closed) when present. | [fixture](tests/fixtures/agent-plugins/v1-portable) · [test](packages/core/tests/agent-plugins/compatibility-status.test.ts) |
...
| `copilot-native-registration` | target-specific | Copilot native Agent Plugins registration writes owned catalog+ledger under apm_modules/.github/plugin/ and merges settings.local.json (@apm); live load from modules with no copy or --plugin-dir; loose skill/MCP projection skipped for admitted roots. | [fixture](tests/fixtures/agent-plugins/v1-portable) · [test](packages/core/tests/acceptance/copilot-native-agent-plugins/install-registration.test.ts) |
