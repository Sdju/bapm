...
- [x] 1.1 Change `createCliIntegrationRegistry` to return an empty registry (remove `@bapm/integration-cursor` static import/register)
- [x] 1.2 Remove `@bapm/integration-cursor`, `@bapm/integration-claude`, `@bapm/integration-codex` from `@bapm/cli` dependencies via pnpm/vp catalog tooling (keep `@bapm/integration-api`); ensure workspace install still links integrations for packages that need them
...
- [x] 2.2 Before pack marketplace emit, dynamically resolve/register `@bapm/integration-claude` / `@bapm/integration-codex` when those formats are selected; fail closed with install guidance if unresolved
...
- [x] 3.1 When `bapm init` records `--target <id>` for the Cursor example (and other `@bapm/integration-<id>` convention ids as designed), emit object-map `targets: { <id>: "@bapm/integration-<id>" }` plus `active: [<id>]` instead of string-only `target:`
...
- [x] 5.2 Update install/compile/MCP/map-load fixtures that assumed built-in Cursor to declare object-map + resolvable `@bapm/integration-cursor` (devDependency or fixture resolve), without re-adding CLI hard dep
...
- [x] 6.2 Confirm `@bapm/cli` package.json has no hard deps on concrete `@bapm/integration-cursor|claude|codex`
