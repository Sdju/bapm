...
- Add a FEOD **Resolver** module in `@b-apm/core`: classify deps (local / git-semver / git-literal / registry-deferred), BFS transitive resolve, OpenAPM **intersection-pick** diamonds (not APM first-wins), depth cap 50, nest refuse, cycle fail-closed
...
- Add thin **`bapm lock`** in `@b-apm/cli` (FEOD command + module) wrapping core; flags subset: `--verbose`, `--update`, `--parallel-downloads`
...
- **Primary:** `@b-apm/core` — new `modules/Resolver` (classify, graph resolve, download port, modules path constant, `resolveAndLock`); consumes Manifest + Lockfile public APIs only (no deep imports)
- **CLI:** `@b-apm/cli` — `commands/lock.ts`, module wrapping core, registry + help update
