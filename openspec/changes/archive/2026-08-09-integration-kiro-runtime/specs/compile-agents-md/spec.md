...
#### Scenario: Kiro-target compile may emit AGENTS.md

- **WHEN** compile runs with the Kiro integration active as the compile target and that integration exposes `compile`
- **THEN** `AGENTS.md` (or the Kiro compile output path) MAY be written by the Kiro host emitter and MUST NOT be treated as a forbidden foreign artifact for that run
