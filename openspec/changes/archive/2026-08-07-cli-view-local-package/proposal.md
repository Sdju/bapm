...
- FEOD CLI: new `View` module + thin `commands/view`; core logic via `@bapm/core` public API (no deep imports)
...
- `@bapm/core`: new View (or Deps-adjacent) public API for local package inspect; reuse lock load + query resolve helpers; reuse existing modules-path location (`locateGitPackageTree` or equivalent); read package `apm.yml`/`bapm.yml` for summary/description when present
