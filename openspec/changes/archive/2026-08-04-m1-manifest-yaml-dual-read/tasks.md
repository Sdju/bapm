...
- [x] 1.1 Add a YAML parsing dependency to `@bapm/core` via pnpm catalog / workspace CLI only (do not hand-edit version pins); follow `.cursor/skills/pnpm-dependencies/SKILL.md`
...
- [x] 4.3 Run `vp run -r build` / `@bapm/core` test + `vp check` (or package-local equivalents) and fix regressions in core only
