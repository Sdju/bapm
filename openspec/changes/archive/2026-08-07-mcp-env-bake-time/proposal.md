...
- `@b-apm/core` — new small bake helper (likely under `Mcp` or `Install`); wire into install → `configureMcp` path.
- `@b-apm/integration-cursor` — ensure written `mcpServers` entries use baked values; tests for placeholder → literal.
