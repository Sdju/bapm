...
Defines `@b-apm/core` install orchestration after M3 resolve/download: modules placement, lock write-back unless frozen, target intersection and deploy only through registered `@b-apm/integration-api` contracts, and basic OpenAPM frozen gate (lk-006).
...
Core install MUST invoke host materialization only through `@b-apm/integration-api` registration/contracts. Core MUST NOT hard-depend on `@b-apm/integration-cursor` or any concrete `bapm-target-*` package. With no target registered or none detected, install MUST still complete modules and lock work and MUST NOT write harness files (MAY warn).

#### Scenario: No hard dependency on concrete target

- **WHEN** inspecting `@b-apm/core` package dependencies
- **THEN** it MUST list `@b-apm/integration-api` (or workspace equivalent) and MUST NOT list `@b-apm/integration-cursor` or other concrete `bapm-target-*` packages
