...
- **WHEN** `@bapm/integration-codex` exposes both marketplace-output and runtime capabilities
- **THEN** pack MUST still be able to select marketplace-output without requiring runtime detect/materialize activation for that pack run
...
- **WHEN** pack runs with only Codex marketplace output enabled and `@bapm/integration-codex` is resolvable and also exports a runtime factory
