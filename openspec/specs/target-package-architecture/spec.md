...
Creating `bapm-target-api` / `bapm-target-*` packages, wiring workspace entries, and implementing materialization MUST wait for an explicit OpenSpec change (or equivalent implementation task). Recording this capability alone MUST NOT create those packages.

#### Scenario: Spec exists without packages on disk
- **WHEN** this capability is present in `openspec/specs/target-package-architecture`
- **THEN** the repository MAY still lack `packages/target-api` / `packages/target-*` until a later change implements them
