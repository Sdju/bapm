...
- **THEN** success criteria MUST be expressed as core↔integration API contracts and optional published `@b-apm/integration-*` packages, NOT as “match APM's N built-in adapters inside core/cli”

### Requirement: Integration packages own host capabilities

Each host integration SHALL be published as `@b-apm/integration-<id>`. An integration MAY provide runtime capabilities (detection, primitive deployment, MCP configuration, or compile emission), marketplace-output capabilities, or both. It MUST own every host-specific layout, document shape, default path, and mapping used by the capabilities it provides. Integration packages MUST depend on the shared integration API package for types and utilities, and `@b-apm/core` MUST NOT embed host-specific deploy or marketplace-output logic.
...
- **THEN** the selected integration supplies the host-specific behavior and `@b-apm/core` uses only generic capability contracts
...
`@b-apm/core` test/vite configuration MUST NOT use a path alias that remaps `@b-apm/integration-cursor` to a filesystem entry under `packages/integration-cursor` as a substitute for package resolution. Optional e2e that needs cursor MUST resolve it via workspace protocol / standard Node resolution from a package that is allowed to depend on cursor (CLI or a dedicated test harness), or avoid importing cursor from core entirely by using a mock integration registered through `@b-apm/integration-api`.

#### Scenario: No vite cursor alias in core

- **WHEN** inspecting `packages/core` vite/test resolve aliases after this change
- **THEN** there MUST be no alias entry whose purpose is to redirect `@b-apm/integration-cursor` into the monorepo source tree for core tests

### Requirement: Concrete integrations own host layout and compile emission

Each concrete `@b-apm/integration-<id>` package MUST own its host-specific detection signals, deploy layout, primitive materialization mapping, MCP configuration layout, and compile output rendering/default path. `@b-apm/core` and the CLI composition root MUST use only generic integration-api contracts for these operations and MUST NOT contain Cursor-specific compile rendering, `AGENTS.md` defaulting, deploy-path attribution, or integration-id allowlists.
...
- **THEN** resolution fails, while `@b-apm/integration-api` and the required concrete `@b-apm/integration-*` packages resolve to their published identities
