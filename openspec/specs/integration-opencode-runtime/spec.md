...
Defines the `@b-apm/integration-opencode` package: OpenCode host detect, skill/agent/command materialize under `.opencode/`, MCP adaptation into project `opencode.json`, and compile to project-root `AGENTS.md` (including instructions), depending only on `@b-apm/integration-api`.
