...
| 5 | `merge` | generalPurpose | Archive/sync OpenSpec + при необходимости влить/оставить тесты |
| 6 | `deliver` | — | Короткий отчёт пользователю **только** из отчётов субагентов |
...
2. Зафиксируй todo по фазам `plan → acceptance → apply → accept → merge → deliver` (и промежуточные commit-шаги после успешных фаз).
...
5. На `deliver` — итог: change name, что сделано, пути acceptance-тестов, статус archive, **хеши/сообщения коммитов по фазам**; без собственного аудита кода.
